@echo off
title AzerothCore Builder 2022
:start
SET mainfolder=%CD%
SET msbuildpath="%mainfolder%\Tools\VisualStudio2022\MSBuild\Current\Bin\msbuild.exe"
SET cmake=3.23.1
set database=MariaDB-10.6.7
set database_lib=libmariadb
set openssl=1.1.0
SET BOOST_ROOT="%mainfolder%\Tools\boost"
SET BOOST_LIBRARYDIR="%mainfolder%\Tools\boost\lib64-msvc-14.3"
...
if exist "%mainfolder%\Tools\VisualStudio2022\MSBuild\Current\Bin\MSBuild.exe" goto git_clone
if not exist %msbuildpath% goto install_vs_community
goto git_clone

:install_vs_community
cls
"%CD%\Tools\vs_community.exe" -p --installPath "%CD%\Tools\VisualStudio2022" --add Microsoft.VisualStudio.Component.VC.Tools.x86.x64 --add Microsoft.VisualStudio.Workload.NativeDesktop;includeRecommended
echo.
...
echo Generate cmake...
echo.
"%mainfolder%\Tools\cmake\%cmake%\bin\cmake.exe" "%mainfolder%/Source/%sourcepath%" -G "Visual Studio 17 2022%arch%" -DBoost_DIR="%mainfolder%\Tools\boost" -DBoost_INCLUDE_DIR="%mainfolder%\Tools\boost" -DOPENSSL_ROOT_DIR="%mainfolder%/Tools/openssl/%openssl%-%archpath%" -DOPENSSL_INCLUDE_DIR="%mainfolder%/Tools/openssl/%openssl%-%archpath%/include" -DMYSQL_LIBRARY="%mainfolder%/Tools/database/%database%-%archpath%/lib/%database_lib%.lib" -DMYSQL_INCLUDE_DIR="%mainfolder%/Tools/database/%database%-%archpath%/include/mysql" -DGIT_EXECUTABLE="%mainfolder%/Tools/Git/bin/git.exe" -DTOOLS=1 -DPLAYERBOT=1 -DPLAYERBOTS=1 -DBUILD_EXTRACTORS=1 -DWITH_CPR=1 -DSCRIPTS=static
echo.
echo Start building...
echo.
...
copy "%mainfolder%\Tools\database\Database.bat" "%mainfolder%\Repack" /Y

cls
echo Starting the database server...
if exist "%mainfolder%\Repack\Database\share\english\errmsg.sys" goto finalize_db
...
copy "%mainfolder%\Tools\database\%database%-%archpath%\bin\libmariadb.dll" "%mainfolder%\Repack" /Y
